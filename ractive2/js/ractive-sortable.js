module.exports = {
  addSortableEvents: function(Ractive) {
    ...
    Ractive.events.sortable = function (node, fire) {
      ...
      node._ractive.root.on('change', function(evt) {
        $self.foreach(node.children, Drag.childDraggable);
      });
      // References
    ...
    Ractive.events.sortable.Drag = Ractive.events.wrap(function (node, fire) {
      ...
      var Class = function (el) {
        var library = {
          has: function (name) {
            if (ClassListSupported && el.classList) return el.classList.contains(name);
      ...
      var Drag = {
        ...
        start: Delegate(function (event) {
          ...
          fire({
              node: node,
              context: {
                target: this._ractive.root.get(this._ractive.keypath.str),
                current: Drag.current.element._ractive.root.get(Drag.current.element._ractive.keypath.str)
              },
              target: this,
        ...
        enter: Delegate(function (event) {
          ...
          if (previous === 0) {
            ...
            fire({
              ...
              context: {
                target: this._ractive.root.get(this._ractive.keypath.str),
                current: Drag.current.element ? Drag.current.element._ractive.root.get(Drag.current.element._ractive.keypath.str): undefined,
              },
              target: this,
              current: Drag.current.element,
              original: event,
        ...
        childDraggable: function(element){
        ...
        attach: function (element) {
          ...
          for (var i = 0; i < element.children.length; i++){
            element.children[i].draggable = true;
          }
          ...
          element.addEventListener('drag', Drag.event('drag_drag'));
        ...
        detach: function (element) {
          ...
          for (var i = 0; i < element.children.length; i++){
            element.children[i].draggable = false;
          }
          ...
          element.removeEventListener('drag', Drag.event('drag_drag'));
